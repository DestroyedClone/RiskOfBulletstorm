...
namespace RiskOfBulletstorm.Items
{
    public class Dog : Item_V2<Dog>
    {
        ...
        {
            var descText = $"{Pct(DogDigChance)} (+{Pct(DogDigChanceStack)} chance per stack) chance every 45 seconds.";
            if (EnableDogPet)
            {
                descText += $"\nInteract to pet.";
            }
        ...
        protected override string GetLoreString(string langID = null) => "Keeps the Huntress company. He has a good nose for treasure, but all attempts to train him in combat have failed.";
        ...
        public class IsBeetle : MonoBehaviour
        {
