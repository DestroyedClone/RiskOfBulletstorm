...
namespace RiskOfBulletstorm.Items
{
    public class Mustache : Item_V2<Mustache>
    {
        ...
        public float MustacheHealAmount { get; private set; } = 0.10f;
        ...
        protected override string GetDescString(string langid = null) => $"Heals for {Pct(MustacheHealAmount)} health per stack.";
        ...
        private static ItemDisplayRuleDict GenerateItemDisplayRules()
        {
            ...
            rules.Add("mdlBrother", new ItemDisplayRule[]
            {
                new ItemDisplayRule
                {
                    ...
                    followerPrefab = ItemBodyModelPrefab,
                    childName = "Head",
                    localPos = new Vector3(0f, -0.08f, 0.2f),
                    localAngles = new Vector3(0f, 0f, 0f),
                    localScale = generalScale
                }
            });
                ...
                if (body)
                {
                    if (body.inventory)
                    {
                        ...
                        if (InventoryCount > 0)
                        {
                            ...
                            //self.costType Think it's fine to let it trigger on blood shrine uses
                            //var ResultHeal = HealAmount * component.maxHealth;
                            if (component)
                            {
                                Debug.Log("InventoryCount = " + InventoryCount + "| HealAmount = " + MustacheHealAmount + " | SHOULD EQUAL "+InventoryCount* MustacheHealAmount);
                                var ResultHeal = MustacheHealAmount * InventoryCount;
                                Debug.Log("ResultHeal: "+ResultHeal);
                                component.healthComponent.HealFraction(ResultHeal, default);
                                Debug.Log("Stache: Player healfraction for " + ResultHeal.ToString(), self);
                            }
                        }
                    }
                }
